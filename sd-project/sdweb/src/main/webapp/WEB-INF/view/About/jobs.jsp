<%@ page language="java" pageEncoding="GBK"%>
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=GBK">
<title>_速达软件(广州)直销中心-官方授权,专业服务！速达软件,速达3000,速达官网,速达软件官网,广州速达软件,速达5000,速达7000</title>
<meta name="keywords" content="">
<meta name="description" content="">
<link href="suda/templets/new/css/index.css" rel="stylesheet"
	type="text/css">
<link href="suda/templets/new/css/style.css" rel="stylesheet"
	type="text/css">
<link href="suda/templets/new/css/lanmu.css" type="text/css"
	rel="stylesheet">
<script src="suda/templets/new/js/menu.js" type="text/javascript"></script>
<script language="javascript" src="suda/templets/new/js/common.js"></script>
</head>
<body>
	<div align="center">
		<div style="background-color: #FFFFFF; width: 984px;">
			<!-- -------------- top -------------- -->
			<%@include file="../../../top.jsp"%>
			<table width="980" align="center" bgcolor="#FFFFFF" border="0"
				cellpadding="0" cellspacing="0">
				<tbody>
					<tr>
						<td valign="top" width="220">
							<!------------- left --------------> 
							<%@include file="aboutLeft.jsp"%>
						</td>
						<td width="8">&nbsp;</td>
						<td class="tdbk" valign="top" width="752">
							<!-- --------------right  start-->
							<table width="100%" border="0" align="center" cellpadding="0"
								cellspacing="0">
								<tr>
									<td height="30" valign="middle" style="padding-top: 3px;">
										▎您当前的位置：<a href="index.do">首页</a> > <a
										href="about.do">关于我们</a> > <a
										href="jobs.do">人才招聘</a> >
									</td>
								</tr>
							</table>
							<table width="97%" border="0" align="center" cellpadding="0" cellspacing="0">
								<tr>
									<td height="1" bgcolor="#CCCCCC"></td>
								</tr>
							</table>
							<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
								<tr>
									<td width="100%" align="center" style="padding-top: 6px;">
										<img src="suda/templets/new/images/banner/12.jpg">
									</td>
								</tr>
							</table>
							<table width="100%" border="0" align="center" cellpadding="15"
								cellspacing="0">
								<tr>
									<td width="729" height="174" valign="top" class="hg">
										<table width="100%" border="0" cellspacing="0" cellpadding="0">
											<tr>
												<td>
													<!--内容调用-->
													<div class="hg">
														<div align="left" style="line-height: 150%">
															<br /> <span style="font-size: 14px"><span
																style="line-height: 150%; color: rgb(51, 51, 51)">沃迪软件充满活力，鼓励专业提升和职业发展。您可以选择这里，梦开始的地方！<br />
																	<br /> 释放您的激情，展现您的才华，这里将给您足够的舞台！
															</span></span><br /> &nbsp;
														</div>
														<div style="line-height: 10px">&nbsp;</div>
														<div
															style="line-height: 22pt; background: rgb(230, 230, 230)">
															<b>实施顾问</b>
														</div>
														<div style="line-height: 22pt">
															专业要求：财经类<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
																&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>工龄要求：2年以上
														</div>
														<div style="line-height: 22pt">
															学历要求：专科以上&nbsp;<span><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></span>年龄要求：25岁
														</div>
														<div style="line-height: 22pt">
															性别要求：无&nbsp;<span><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
															</span></span>工作性质：培训、实施&nbsp;<span><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></span>
														</div>
														<div style="line-height: 22pt">
															工作区域：广州&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span>&nbsp;
															</span>薪水水平：面议
														</div>
														<div
															style="line-height: 22pt; background: rgb(230, 230, 230)">
															工作简介及其它要求：</div>
														<div style="line-height: 22pt; text-indent: 21pt">
															1、知识要求：大专以上学历，工商管理、信息管理与信息系统、财经类相关专业；专业知识要求：了解基础会计学、市场营销学、经济学、SQL
															Server、企业信息化、ERP、有一定的计算机基础；具有良好的沟通、学习能力；</div>
														<div style="line-height: 22pt; text-indent: 21pt">
															2、工作要求：有财务软件,<strong>进销存软件</strong>，ERP软件销售及实施经验，或者过从事过会计工作；该岗位必须要求有一定会计工作或财务软件实施经验；要求可以给客户做财务软件操作培训，说话声音要求宏亮或意思表达清晰；条件不附者勿投简历。
														</div>
														<div style="line-height: 22pt; text-indent: 21pt">
															3、待遇：底薪+绩效工资+业绩提成</div>
														<div style="line-height: 15pt">&nbsp;</div>
														<div
															style="line-height: 22pt; background: rgb(230, 230, 230)">
															<b>销售业务</b>
														</div>
														<div style="line-height: 22pt">
															专业要求：财经类或计算机类<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
															</span>工龄要求：1年以上
														</div>
														<div style="line-height: 22pt">
															学历要求：专科以上<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>年龄要求：22-40岁
														</div>
														<div style="line-height: 22pt">
															性别要求：无&nbsp;<span><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span></span>工作性质：销售财务软件或ERP软件<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
															</span>
														</div>
														<div style="line-height: 22pt">
															工作区域：广州&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;薪水水平：面议</div>
														<div
															style="line-height: 22pt; background: rgb(230, 230, 230)">
															工作简介及其它要求：</div>
														<div style="line-height: 22pt; text-indent: 21pt">
															&nbsp;1、大专以上学历，工商管理、信息管理与信息系统、市场营销、财会相关专业；专业知识要求：基础会计或以上、市场营销学、经济学、SQL
															Server、企业信息化、ERP、熟悉计算机基础；</div>
														<div style="line-height: 22pt; text-indent: 21pt">
															2、有一定的商务谈判能力，能吃苦耐劳；有一定的客户开发及电话营销经验；具有良好的沟通、学习能力；</div>
														<div style="line-height: 22pt; text-indent: 21pt">
															3、具有会计基础知识者优先；</div>
														<div style="line-height: 22pt; text-indent: 21pt">
															4、待遇：底薪+绩效工资+业绩提成<span style="display: none">&nbsp;</span>
														</div>
													</div> <br /> <br />
													<p class="hg">
														<strong>速达软件（广州）销售服务中心</strong><br />
														办公地址：广州市天河区五山路263号瑞华大厦南塔16B房<br />店面地址：广州市天河区天河路502号天河电脑城二楼258A<br /> 销售热线1：400-082-8868<br />销售热线2：020-85589742<br />
														服务热线：020-85589743<br /> 维护专线：020-85589747<br />
														
													<table width="300" border="0" cellspacing="0"
														cellpadding="0">
														<tr>
															<td width="51" height="22"><a
																href="tencent://message/?uin=4000828868&amp;Site=速达软件咨询&amp;Menu=yes"><img
																	src="suda/templets/new/images/pa.gif"
																	width="51" height="16" style="border: 0" /></a></td>
															<td width="80" valign="bottom">：<a
																href="tencent://message/?uin=4000828868&amp;Site=速达软件咨询&amp;Menu=yes">4000828868</a></td>
															<td width="51"><a
																href="tencent://message/?uin=12361918&amp;Site=速达软件咨询&amp;Menu=yes"><img
																	src="suda/templets/new/images/pa.gif"
																	width="51" height="16" style="border: 0" /></a></td>
															<td valign="bottom">：<a
																href="tencent://message/?uin=12361918&amp;Site=速达软件咨询&amp;Menu=yes">12361918</a></td>
														</tr>
													</table>
													</p>
													<p align="center" class="pageLink"></p>
												</td>
											</tr>
										</table>

									</td>
								</tr>
							</table> 
							<!-----------------------------------right  end------->
						</td>
					</tr>
				</tbody>
			</table>
			<!-- 页脚 -->
			<!-- -------------------foot---------------- -->
			<%@include file="../../../foot.jsp"%>
		</div>
	</div>
</body>
</html>