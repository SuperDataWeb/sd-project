...
<BODY type="leaveMessage">
	<table cellpadding="0" cellspacing="15" border="0" align="center" width="100%">
        ...
        <tbody><tr>
          <td class="hg" height="174" valign="top" width="729"><table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tbody><tr>
                <td height="50" align="center"><span class="title">分公司性质销售企业</span></td>
              </tr>
            </tbody></table>
	...
	<strong>客户简介</strong>：<br>
	　　渝次油研济南分公司经营性质属于分公司，同时掌管山东境内及周边销售网络，公司内部制度化建设基本健全，现已组建局域网，但没有信息化管理的经验。<br>
	管理软件需求：<br>
	　　以销售为核心，通过采用信息化技术，梳理公司物流。信息数据真实、实时反映企业库存货品与业务需求的动态关系，同时要求软件具备成熟的技术。<br>
	　　1.大约十个点，要求不同的操作员拥有不同的权限，权限的设置可以先定义工作组权限，再在组内定义操作员权限。<br>
	　　2.货品查询：支持多检索方式查询，如编码、名称、规格等，实现所查货品总量显示以及分仓库分部列表。<br>
	　　3.单据格式：各种单据可以设置相应个性化格式，可以调出可复制可编辑的单据样式。<br>
	　　4.赠品的处理：有些大客户团购时会赠送些某些商品作为促销，要求能够随时查到赠品的发出情况，赠品平时是作为销售的，要求只查赠品（销售除外）一个时段的发出数量等情况。<br>
	　　5.销售管理：通过销售管理建立客户档案，健全客户信用监控体系；建立对客户订单的动态管理，快速响应客户订单，及时处理销售退/换货需求；建立灵活的销售价格策略和销售折扣管理；销售发票、应收款、成本核算与财务系统集成。<br>
	　　6.库存管理：通过库存物品的入库、出库、移动和盘点等操作对企业的物流进行全面的控制和管理，提供物料的分级、分类管理；支持对库存物品的批次、单件、保质期管理方式；能够最大安全库存量，超限报警处理、积压/短缺统计；可以动态地查看各种物料库存、各类进/出情况等。<br>
	　　7.采购管理：完善供应商档案管理，采购发票、应付款、成本核算等财务系统集成。<br>
	　　8.财务管理：希望实现财务系统与采购、销售、应收应付系统集成，实现物流、资金流、信息流的完美协调。<br>
	<br>
	<strong>案例解析</strong>：<br>
	　　对管理软件的要求分为10个点，总体来说客户需求的软件为速达3000XP10用户。<br>
	　　1．在操作员定义与授权中，先定义好组及组的权限，然后新增操作员，并选择操作员隶属于哪个组。<br>
	　　2．在库存报表中可以实现货品多检索方式查询查询：如编码、名称、规格等，并可以实现所查货品总量显示以及分仓库分部列表。<br>
	　　3．在单据录入页面上方幽模板设计，可以针对不同的单据设置不同的样式，比如销售开单，在模板样式设计中设计打印样式，右边有样式的导入和导出按钮，可以导出*.zrp格式的文件，此文件可以应用于以后的帐套样式设计中。<br>
	　　4．对于赠品的处理可以采用销售时赠品售价为零，然后做一个自定义报表(从速达3000xp开始软件中加入了自定义业务报表功能)，名称可以叫做赠品发送数量表，定义过滤条件是售价为零，同时过滤条件中加入查询起始时间和结束时间，这样就可以查询一个时段赠品的发出数量。<br>
	　　5．采用财务与进销存相结合的方式，销售发票、应收款、成本核算与财务系统集成。在客户资料中设置客户的信誉额度，健全客户信用监控体系；设置超期应收款及超期订单的警报功能，在启动软件时可以自动弹出已经超期的各个单据，以此建立对客户订单的动态管理，快速响应客户订单，及时处理销售退/换货需求；针对不同的客户级别设置不同级别的销售价格，建立起灵活的销售价格策略；在资料→货品→商品限时打折资料立可以对销售的客户进行折扣的管理。<br>
	　　6．在货品资料设置时，可针对不同的货品设置不同的存货类型和货品类型，以及成本核算方法。对库存物品的批次、单件、保质期管理方式可以通过个别指定法实现，将每个货品的批次号按照日期进行编制可实现保质期的管理方式；对每个货品设置最大/小库存量，系统会根据当前的库存量给予相应的警报或是提示；通过库存报表可以动态地查看各种物料库存、各类进/出情况等。<br>
	　　7．在供应商资料中建立供应商的所有基础资料，便于对供应商进行查询管理。采用财务与进销存相结合的方式，可以对销售业务和帐务进行挂接。<br>
	　　8．采用财务与进销存相结合的方式，可实现财务系统与采购、销售、应收应付系统集成，实现物流、资金流、信息流的完美协调<span style="display: none">&nbsp;</span></p>
<p>
	<strong>速达官网</strong>销售的<strong>速达</strong><strong>3000pro</strong>，<strong>速达</strong><strong>3000xp</strong>，<strong>速达</strong><strong>5000</strong>，<strong>速达</strong><strong>7000</strong>，都能很好的处理分公司性质销售企业解决方案</p>
<p>
