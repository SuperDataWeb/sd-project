<%@ page language="java" pageEncoding="GBK"%>
<%@ include file="/WEB-INF/view/include/commonjslib.jsp" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3c.org/TR/1999/REC-html401-19991224/loose.dtd">
<!-- saved from url=(0157)http://chat10.live800.com/live800/chatClient/chatbox.jsp -->
<HTML xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=GBK">
<title>中小企业管理_速达软件(广州)直销中心-官方授权,专业服务！速达软件,速达3000,速达官网,速达软件官网,广州速达软件,速达5000,速达7000</title>
<meta name="keywords" content="">
<meta name="description" content="">
<link href="suda/templets/new/css/index.css" rel="stylesheet" type="text/css">
<link href="suda/templets/new/css/style.css" rel="stylesheet" type="text/css">
<link href="suda/templets/new/css/lanmu.css" type="text/css" rel="stylesheet">
<script src="http://lxbjs.baidu.com/lxb.js?sid=1272542" async="" charset="utf-8" type="text/javascript"></script>
<script	src="http://qiao.baidu.com/site/886/4e9275315db365e8bfde1fe0e12128f9/b.js" async="" charset="utf-8" type="text/javascript"></script>
<script src="suda/templets/new/js/menu.js" type="text/javascript"></script>
<script language="javascript" src="suda/templets/new/js/common.js"></script>
<script src="http://qiao.baidu.com/v3/asset/js/bw.js?v=20140515" charset="UTF-8" type="text/javascript"></script>
<link charset="utf-8" type="text/css" href="http://lxbjs.baidu.com/float/asset/1001.css" rel="stylesheet">
<link href="http://qiao.baidu.com/v3/asset/css/m-front-icon.css?v=20140626"	type="text/css" rel="stylesheet">
<script charset="UTF-8"	src="http://r.qiao.baidu.com/Enter.php?callback=BDBridge.rcvbase.__handleEnter&amp;siteid=1272542&amp;bid=&amp;referrer=&amp;word=&amp;coding=&amp;bdclkid=&amp;title=%E4%B8%AD%E5%B0%8F%E4%BC%81%E4%B8%9A%E7%AE%A1%E7%90%86_%E9%80%9F%E8%BE%BE%E8%BD%AF%E4%BB%B6(%E5%B9%BF%E5%B7%9E)%E7%9B%B4%E9%94%80%E4%B8%AD%E5%BF%83-%E5%AE%98%E6%96%B9%E6%8E%88%E6%9D%83%2C%E4%B8%93%E4%B8%9A%E6%9C%8D%E5%8A%A1%EF%BC%81%E9%80%9F%E8%BE%BE%E8%BD%AF%E4%BB%B6%2C%E9%80%9F%E8%BE%BE3000%2C%E9%80%9F%E8%BE%BE%E5%AE%98%E7%BD%91%2C%E9%80%9F%E8%BE%BE%E8%BD%AF%E4%BB%B6%E5%AE%98%E7%BD%91%2C%E5%B9%BF%E5%B7%9E%E9%80%9F%E8%BE%BE%E8%BD%AF%E4%BB%B6%2C%E9%80%9F%E8%BE%BE5000%2C%E9%80%9F%E8%BE%BE7000&amp;lv=0&amp;lc=0&amp;ls=&amp;lvp=0&amp;vis_type=3&amp;ftime=0&amp;fs=&amp;ltime=0&amp;lang=zh-CN&amp;rsl=1920*1080&amp;tz=UTC%2B9%3A0&amp;cbit=24&amp;t=1412950588703"
	id="BDBridgeSendData" language="javascript" type="text/javascript"></script>
<link href="http://qiao.baidu.com/v3/asset/css/m-front-mess.css?v=20130723"	type="text/css" rel="stylesheet">
<link href="http://qiao.baidu.com/v3/asset/css/m-front-invite.css?v=20130705" type="text/css" rel="stylesheet">
<style>
@import "http://qiao.baidu.com/v3/asset/css/m-webim-lite.css?v=20130705"
	;
</style>
<script charset="UTF-8"
	src="http://r.qiao.baidu.com/Refresh.php?callback=BDBridge.rcvbase.__handleRefresh&amp;bid=4ea016ca5ce9420ac90e8dbc&amp;siteid=1272542&amp;t=1412950621171"
	id="BDBridgeReport" language="javascript" type="text/javascript"></script>
</head>
<BODY type="leaveMessage">
	<table cellpadding="0" cellspacing="15" border="0" align="center" width="100%">
        
        <tbody><tr>
          <td class="hg" height="174" valign="top" width="729"><table width="100%" border="0" cellpadding="0" cellspacing="0">
              <tbody><tr>
                <td height="50" align="center"><span class="title">食品批发行业</span></td>
              </tr>
            </tbody></table>
              <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tbody><tr>
                  <td height="26" align="center" bgcolor="#E7F1E8">作者：<a href="index.htm">广州速达</a>&nbsp; &nbsp;来源：速达软件官网 &nbsp; &nbsp;时间2012-02-05&nbsp; &nbsp;阅读：
                   <script src="/suda/plus/count.php?view=yes&amp;aid=4104&amp;mid=0" type="text/javascript" language="javascript"></script>237 次</td>
                </tr>
              </tbody></table>
            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tbody><tr>
                  <td height="10">&nbsp;</td>
                </tr>
                <tr>
                  <td><!--内容调用-->
                    <p class="hg">　　
                    </p><p>
	客户简介：</p>
<p>
	　　嘉禾商贸是当地康师傅方便面、饼干、饮料的最大代理商，同时代理统一的方便面和饮料系列，另外还代理其品牌商品近千种。主要以给他的二级代理商（其它商贸公司）、各大商场，超市，小卖点送货为主，自营一500平的超市，超市主要经营其自已代理的商品和散装的饼干和面包。因管理需要想购买一套管理软件。要求管理软件具备应有的进、销、存和简单的财务功能外，还有一些自已的要求。管理软件需求如下：<br>
	　　1、以上提及的业务要求在一个帐套内全部管理起来；<br>
	　　2、因同业竞争的原因，康师傅公司不允许其在所用的销售开单上出现统一产品的字样，在没上管理软件之前其是另外起了一个名为福豆粒商贸的公司在开统一公司产品的销售单，现要求管理软件能在同一账套下即能开嘉禾商贸的单子也能开福豆粒的单子；<br>
	　　3、因其送货客户种类多，所以价格分类也特别多，总共有十个级别的客户，每级客户的价格均不一致。再加上其自已的商场要有一个价格；<br>
	　　4、嘉禾商贸原来用的其它品牌的进销存进行管理，该进销存用的ACCESS数据库；嘉禾商贸提出能不能简化初始化过程；<br>
	　　5、嘉禾商贸的老板因业务太忙，需要具有一个高度集成的查询平台进行查询公司的经营情况：如哪个公司欠款、哪个产品卖的好、哪个产品带来的利润大等以利于其进行决策；<br>
	　　6、嘉禾商贸也经常会跟他的二级代理商发生一些采购业务；并且其跟商场的合作主要分为三种：一、现款；二、月结；三、委托代销；<br>
	　　7、嘉和商贸进行信息化的经费要求控制在2万元以下（其原有电脑三台，自营的商场此次对于散货部份先不进行管理，但两个月后此系统正常后再进行信息化）。<br>
	　　<br>
	　　建议解决方案：</p>
<p>
	　　要求：为嘉禾商贸出一个信息化方案（用我们的哪个产品）并说明理由案例解析：为嘉禾商贸推荐速达3000XP网络版5用户，因为3000XP即能满足批发的需求又能满足自营超市的管理（POS），而且价格也能满足嘉禾的需求。具体问题解决如下：<br>
	　　1、嘉禾商贸的所有问题都能在一个账套解决；使用财务进销存结合使用。<br>
	　　2、在销售开单时设计两个打印样式，一个为嘉禾商贸样式，一个为福豆粒商贸样式，在开康师产品的时候选择嘉禾商贸样式，开统一产品时采用福豆粒商贸样式；<br>
	　　3、可以在初始化的时候将价格跟踪方式选为：以最近价，或在〖帐套选项〗里的［当前价格跟踪方式］选择〔以最近价〕跟踪（说明：因为软件在价格跟踪方式这里一共有三种方式：参考价，分级价，最近价，并且后一种价格跟踪方式是包含前一种的所以此案例具体解决方式如下，其自营的商场的价格在录入货品资料时在参考售价即可；对于业务往来频繁的重要客户可事先分出五个级别用客户分级，这样直接就可以实现价格分级；对于其他的业务往来不是特别的频繁的客户，只要在第一次进行业务交易的时候输入一次价格，软件就会一直能记住这个价格）<br>
	　　4、因其他软件用的是ACESS数据库，所以可以将其软件里的客户资料，供应商资料，货品资料导出成EXCEL文件，通过整理，将以上三种资料导入到速达软件里以减化初始化流程。<br>
	　　5、可以用3000XP的数据中心功能来满足嘉禾老板的需求。因为数据中心里面包涵了老板所关心的所有数据，里面一共有七个数据中心，企业，客户，客户明细，供应商，供应商明细，货品，货品明细，可能让负责人查到所有其想要的数据；<br>
	　　6、对于跟二级代理商发生采购业务在3000XP中可以应用双重身份设定功能，现款销售时用现款销售功能；月结代理商用销售开单功能，收款期限填上数字30即可；代销的业务可通过委托代销功能进行实现；<br>
	　　7、因为3000XP5用户的价格是12800元，其还有7200元余额，再购置两台配置一般的台式机，自组装好POS用的打印机和钱箱，金额刚好；<br>
	　　8、备注：此答案并不是唯一选择，只是我们认为此答案是最佳选择（客户也是选择的3000XP5用户）比如3000.NET5用户也能满足用户需求</p>
<p>
	<strong>速达软件官网</strong>销售的<strong>速达</strong><strong>3000pro</strong>，<strong>速达</strong><strong>3000xp</strong>，<strong>速达</strong><strong>5000</strong>，<strong>速达</strong><strong>7000</strong>，都能很好的处理食品批发行业解决方案</p>
<p>
	keywords:<a href="http://www.buyerp.cn/"><strong>广州速达</strong>,<strong>速达软件官网</strong>,<strong>速达财务</strong>,<strong>速达</strong><strong>3000pro</strong>,<strong>速达</strong><strong>3000xp</strong>,<strong>速达</strong><strong>7000</strong>,<strong>速达软件</strong>,<strong>速达社区</strong>,<strong>速达</strong><strong>V</strong></a></p>
<p>
	&nbsp;</p>

                     <p></p>
                    <br><br>     
					<div id="ckepop">
	<span class="jiathis_txt">分享到：</span>
	<a title="分享到QQ空间" class="jiathis_button_qzone"><span class="jiathis_txt jiathis_separator jtico jtico_qzone">QQ空间</span></a>
	<a title="分享到新浪微博" class="jiathis_button_tsina"><span class="jiathis_txt jiathis_separator jtico jtico_tsina">新浪微博</span></a>
	<a title="分享到人人网" class="jiathis_button_renren"><span class="jiathis_txt jiathis_separator jtico jtico_renren">人人网</span></a>
	<a title="分享到开心网" class="jiathis_button_kaixin001"><span class="jiathis_txt jiathis_separator jtico jtico_kaixin001">开心网</span></a>
	<a href="http://www.jiathis.com/share" class="jiathis jiathis_txt jiathis_separator jtico jtico_jiathis" target="_blank">更多</a>
	<a class="jiathis_counter_style"><span title="累计分享0次" id="jiathis_counter_43" class="jiathis_button_expanded jiathis_counter jiathis_bubble_style">0</span></a>
</div>
<script type="text/javascript" src="http://v2.jiathis.com/code_mini/jia.js" charset="utf-8"></script>
                    <br> <br>
                   <p class="hg"> <strong>速达软件（广州）销售服务中心</strong><br>
  地址：广州市番禺区大石588号金怡商业大厦301室<br>
  销售热线：400-034-3668<br>
  服务热线：020-85279086<br>
  维护专线：020-31001310<br>
  VIP 专线：18620607677　18127974810
</p><table width="300" border="0" cellpadding="0" cellspacing="0">
  <tbody><tr>
    <td height="22" width="51"><a href="tencent://message/?uin=1601331518&amp;Site=速达软件咨询&amp;Menu=yes"><img src="/suda/templets/new/images/pa.gif" style="border:0" height="16" width="51"></a></td>
    <td valign="bottom" width="80">：<a href="tencent://message/?uin=1601331518&amp;Site=速达软件咨询&amp;Menu=yes">1601331518</a></td>
    <td width="51"><a href="tencent://message/?uin=4000343668&amp;Site=速达软件咨询&amp;Menu=yes"><img src="/suda/templets/new/images/pa.gif" style="border:0" height="16" width="51"></a></td>
    <td valign="bottom">：<a href="tencent://message/?uin=4000343668&amp;Site=速达软件咨询&amp;Menu=yes">4000343668</a></td>
  </tr>
</tbody></table>
<p></p>
                    <p class="pageLink" align="center"></p></td>
                </tr>
          </tbody></table>
          
          </td>
        </tr>
      </tbody>
	</table>
	<!-- /right -->
</BODY>
</HTML>
